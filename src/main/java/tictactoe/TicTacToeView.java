...
/**
This class represents the graphical view of the X-O tic tac toe game. The class will generate a button
grid for the user to interact with, call methods from the tic tac toe game to check win and take turns. 
...
public class TicTacToeView extends JPanel{
    ...
    public TicTacToeView(GameUI main){
        super();
        player1 = new Player();
        player2 = new Player();
        ...
        root = main;
        add(new JLabel("Welcome to Tic Tac Toe!"),BorderLayout.NORTH);
        ...
        for (int i = 0; i < 3; i++){
            for (int j = 0; j < 3; j++){
                ...
                buttons[i][j].addActionListener(e -> {
                                                      inputSymbol(game.getCurrentPlayer(),e);
                                                      checkGameState();});
    ...
    private void inputSymbol(int currentPlayer, ActionEvent e){
        updateButtonGrid();
        PositionAwareButton clickedButton = ((PositionAwareButton)(e.getSource()));

        if (currentPlayer == 1){
              game.takeTurn(clickedButton.getAcross(),clickedButton.getDown(),"X");
              clickedButton.setText(game.getCell(clickedButton.getAcross(),clickedButton.getDown()));
        }else{
            game.takeTurn(clickedButton.getAcross(),clickedButton.getDown(),"O");
            clickedButton.setText(game.getCell(clickedButton.getAcross(),clickedButton.getDown()));
        }
        displayGameStateMessage();
